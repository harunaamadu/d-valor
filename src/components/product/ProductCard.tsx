...
export interface ProductCardProps {
  ...
  rating?: number;
  reviewCount?: number;
  /** Variant id for add-to-cart (defaults to id) */
  variantId?: string;
  stock?: number;
  ...
  return (
    ...
    >
      ...
      <Link href={`/product/${slug}`} className="block relative overflow-hidden bg-primary/4 aspect-3/4">
        ...
        {/* Action panel — slides up on hover */}
        <AnimatePresence>
          {hovered && (
            <motion.div
      ...
      <div className="flex flex-col gap-1.5 pt-4">
        ...
        <div className="flex items-start justify-between gap-2">
          <Link href={`/product/${slug}`}>
            <h3 className="font-heading text-base leading-tight text-primary hover:text-accent transition-colors duration-300 line-clamp-2">
              {name}
            </h3>
          </Link>
        </div>
